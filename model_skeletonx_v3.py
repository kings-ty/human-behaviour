...
class MultiScale_TemporalConv(nn.Module):
    ...
    def forward(self, x):
        ...
        # Check sizes before cat (Fix for stride mismatch)
        target_size = branch_outs[0].size()[-2:]
        for i in range(1, len(branch_outs)):
            if branch_outs[i].size()[-2:] != target_size:
                branch_outs[i] = F.interpolate(branch_outs[i], size=target_size, mode='nearest')
...
class SkeletonX_Model(nn.Module):
    def __init__(self, num_class=30, num_point=17, num_person=1, in_channels=2, graph_args=dict(), base_channel=64, adaptive=True):
        super(SkeletonX_Model, self).__init__()

        from train_ctr_gcn import Graph
        ...
        # Exact match to SkeletonX ctrgcn.py build_basic_blocks
        ...
        self.fc = nn.Linear(self.base_channel * 4, num_class)
    ...
    def forward(self, x):
        # x: N, C, T, V
        N, C, T, V = x.size()
        # Input permute: (N, C, T, V) -> (N, M*V*C, T) for BN
        # Assuming M=1
        x = x.permute(0, 1, 3, 2).contiguous().view(N, C * V, T)
        x = self.data_bn(x)
        # Restore: (N, M, V, C, T) -> (N, C, T, V) for layers
